# Federation Bundle Profiles: Technical Reference Guide

## Executive Summary

Directory supports two federation bundle profiles for secure trust bundle exchange between SPIRE servers. Each profile implements distinct security models, infrastructure requirements, and operational characteristics. This document provides comprehensive technical guidance to assist in profile selection and implementation.
...
└──────────┬──────────┘
           ...
           │ (validates DNS SAN: spire.example.com)
           │
           ↓
┌─────────────────────────────────────────────┐
│  NGINX Ingress Controller                   │
│  • Terminates external TLS (Let's Encrypt)  │
│  • Performs traffic inspection/logging      │
│  • Initiates new TLS to backend (bridging)  │
└──────────┬──────────────────────────────────┘
           │
           │ Internal HTTPS Connection
           │ (with SNI: spire.example.com)
           │
           ↓
┌─────────────────────┐
│  SPIRE Server       │
...
└──────────┬──────────┘
           │
           │ SPIFFE mTLS Request
           │ (validates SPIFFE ID URI SAN + bootstrap bundle)
           │
           ↓
┌─────────────────────────────────────────────┐
│  NGINX Ingress Controller                   │
│  • SSL Passthrough mode enabled             │
│  • Forwards encrypted traffic as opaque     │
│  • No inspection, logging, or termination   │
└──────────┬──────────────────────────────────┘
           │
...
spire-server:
  ...
  federation:
    ...
    ingress:
      ...
      tlsSecret: "spire-server-federation-cert"  # legit:ignore
      annotations:
        # Rate limiting
...
3. Update SPIRE server configuration:
   ...
   ingress:
     tlsSecret: "spire-server-federation-cert"  # legit:ignore (Add this line)
     annotations:
...
Both https_web and https_spiffe profiles provide production-ready federation capabilities with distinct operational and security characteristics:

**https_web Profile:**
- Optimized for standard cloud deployments
- Maximizes compatibility with enterprise infrastructure
- Simplifies operational procedures and troubleshooting

**https_spiffe Profile:**
- Optimized for zero-trust architectures
- Eliminates external CA dependencies
- Provides maximum encryption and confidentiality

Profile selection should be based on organizational infrastructure capabilities, security requirements, and operational maturity. Both profiles are fully supported, well-documented, and successfully deployed in production Directory environments.

For organizations without specific architectural constraints, https_web is recommended as the initial deployment profile, with migration to https_spiffe available if requirements evolve.
